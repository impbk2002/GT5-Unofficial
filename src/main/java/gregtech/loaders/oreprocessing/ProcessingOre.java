...
public class ProcessingOre implements gregtech.api.interfaces.IOreRecipeRegistrator {
    ...
    public void registerOre(OrePrefixes aPrefix, Materials aMaterial, String aOreDictName, String aModName, ItemStack aStack) {
        ...
        int multiplier = (aPrefix == OrePrefixes.oreDense) ? 2:1;
        if (aMaterial == Materials.Oilsands) {
            ...
            registerStandardOreRecipes(aPrefix, aMaterial, GT_Utility.copyAmount(1L, aStack), Math.max(1, gregtech.api.GregTech_API.sOPStuff.get(gregtech.api.enums.ConfigCategories.Materials.oreprocessingoutputmultiplier, aMaterial.toString(), 1)) *multiplier* (tIsRich ? 2 : 1));
        }
    }
    ...
    private boolean registerStandardOreRecipes(OrePrefixes aPrefix, Materials aMaterial, ItemStack aOreStack, int aMultiplier) {
        ...
        if(aPrefix != OrePrefixes.ore) {
        	GT_ModHandler.addExtractionRecipe(aOreStack, GT_OreDictUnificator.get(OrePrefixes.ore, aMaterial, aMultiplier));
        }
